    ...
    def wbc_scan(self, min_wbc=97, max_triggers=50, max_wbc=130, plot=False):
        ...
        for wbc in xrange(min_wbc, max_wbc):  # loop over wbc
            ...
            for event in self.get_data(max_triggers):
                trigger_phases[event.triggerPhase] += 1
    ...
    def load_mask(self, file_name):
        ...
        for i, line in enumerate(lines):
            if not line or line.startswith('#'):
                continue
            data1 = [l for l in line.split(' ') if l]
            if data1[0] == 'cornBot':
                data2 = [l for l in lines[i + 1].split(' ') if l]
                i2c = int(data1[1])
...
if __name__ == '__main__':
    ...
    if args.wbc:
        # z.load_mask('/home/testbeam/data/PadTel2019/S129-SiD2_2019-08-22_06-25-14.msk')
