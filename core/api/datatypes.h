...
namespace pxar {
  ...
  pixel(uint32_t rawdata, uint8_t rocid, bool invertAddress=false, bool linearAddress=false) : _roc_id(rocid), _buffer_corruption(false), _invalid_address(false),
     ...
     */
    pixel(std::vector<uint16_t> analogdata, uint8_t rocid, int16_t ultrablack, int16_t black, float l1Offset=0) : _roc_id(rocid) { decodeAnalog(analogdata, ultrablack, black, l1Offset); }
   ...
   */
     ...
     *  case of a failed decoding attempts.
     */
    void decodeAnalog(std::vector<uint16_t> analog, int16_t ultrablack, int16_t black, float level1Off=0);
    ...
    uint8_t translateLevel(int16_t value, uint16_t level1, uint16_t levelS, int16_t black, float level1Off=0, bool isC1R2=false);
    ...
    int16_t adjustLevel(int16_t analog, int16_t level1, int16_t levelS, uint8_t lastLevel);
